...
//			请求转发前后的代码会正常执行的，所以应该在校验为空时结束方法的执行
		...
		if(user1!=null) {
			request.setAttribute("a1","用户已存在");
			request.getRequestDispatcher("/web/adminpersonage.jsp").forward(request, response);
			return  ;
		}
		
				int num = userService.add(job,rsUsername,rsPassword,rsGender,rsPhone,rsAddress);
				if(num==0) {
					request.setAttribute("a1", "增加联系人失败");
					request.getRequestDispatcher("/web/adminpersonage.jsp").forward(request, response);
					return;
				}else {
					request.setAttribute("a1", "增加联系人成功");
					request.getRequestDispatcher("/web/adminpersonage.jsp").forward(request, response);
					return;
				}
	}
